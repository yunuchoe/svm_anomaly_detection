...
This project is a machine learning pipeline, collaboratively designed with Abdel Yasser Mohamed, to detect visual anomalies in images. Rather than training a deep classifier from scratch, this model uses a pre-trained CNN as a feature extractor, paired with a classical One-Class Support Vector Machine for decision boundary formulation.
...
5. **Model Training:** A One-Class SVM (nu=0.01, kernel='rbf', gamma='scale') is trained exclusively on standardized nominal features to learn the bounding distribution of "normal" data.
...
This also includes a tuning script to evaluate pooling dimensions (`pool_dim`: 32, 56, 64, 128) and SVM boundaries (`nu`: 0.01, 0.05, 0.1) across three distinct MVTec datasets (Screws, Pasta, Capsules). Note, the Capsules class was introduced at a later stage to test the models ability to generalize to unseen data.
